...
This template aims to facilitate the writing and sharing of
publications which are based on any kind of analysis data. I found
that often I would have to go and update numbers or figures in a
drafted manuscript as the analysis data changed. It often happened
that I wondered if I had already updated a number I stated or I could
no longer figure out where a specific value came from.

The solution is simple: the analysis code should not only create the
figures and results, but also write them to ~LaTeX~ variables / files
which I could input into the manuscript. This way, the numbers are
always up-to-date, and if I or anyone else has a doubt about a
specific number, I can look at the code which generated it (rather
than trying to retrace my own analysis months later).
...
to [[https://arxiv.org][arXiv]]. These source archives automatically include only files
...
The ~python~ part of the template. The scripts create and compute the figures, tables, and numbers presented in the article using the analysis results in the [[data/]] directory and store them in the [[gfx/]], [[tables/]], and [[stats.tex][aux/stats.tex]] directories / file.

The [[figure_setup.py]] file handles the ~matplotlib~ default settings which
...
It depends on the document style file *and will in general be different between the preprint and the publisher version.* Remember to adapt this value before recompiling the document and submitting to a new destination.
...
SETUP = {
    ...
    "patch.antialiased"    : True,          # render patches in antialised
...
This script creates the figures included in the paper.

There are several tricks to follow which make it easy to trace the figure creation and later understand where they come from:

- Give figures a name (à la ~histogram_bsolute_magnitude~) and keep the name constant in the ~python~ script, the ~LaTeX~ manuscript, and the filename
- Define one function which creates one figure for each figure
- Build the script around command line arguments
...
Now we define functions, one for each figure. The function has the same name as the figure file and label in the manuscript.

Note that the figure created with the ~plt.subplots()~ command has a
default size of one column width. The file format (e.g. ~pgf~, ~pdf~) is added as an argument
of the function to allow easy CLI access.
...
def hlines_demo(FORMAT):
    ...
    plt.tight_layout()
    fig.savefig(f"../gfx/hlines_demo.{FORMAT}")
#+end_src

Two more figure functions, one square and one double-column one.
...
def solution_space(FORMAT):
    ...
    plt.tight_layout()
    fig.savefig(f"../gfx/solution_space.{FORMAT}")
...
def matplotlib_release_dates(FORMAT):
    ...
    plt.tight_layout()
    fig.savefig(f"../gfx/matplotlib_release_dates.{FORMAT}")
#+end_src



Now we add the command line logic. The idea is to have the figure name
as mandatory argument, the format is optional. This could be extended
to accept multiple figures and formats at once.
...
amount of arguments. If the script is called with the ~--all~ flag, all
figures are recompiled.
...
Statistics refers to numbers which may be included in the text but are derived from the data. An example would be a sample size, which may change throughout the analysis and the editing of the manuscript. It is easy to lose oversight of all variables and be certain that the ones in the manuscript are correct and up to date. Another common scenario is coming back to the article months / years later to see where a number comes from. If it was computed and placed in the manuscript by code, this is trivial. If it was hard-written into the manuscript, it is not obvious.

#+begin_src python :tangle compute_stats.py
import os
import sys
...
I am not aware of any package which makes it easy to write ~python~ variables to ~LaTeX~ code, so I write my own helper function.

#+begin_src python :tangle compute_stats.py
def write_variable_to_latex(name, value):
     """Write the variable to the stats.tex file.

     Parameters
     ==========
     name : str
         The LaTeX variable name to be defined. Cannot contain underscores.
     value : str, int, float
         The variable value to assign.
    """
def write_stats_to_file(stats):
    """Writes the tables/stats.tex file."""
    path_stats = os.path.join(os.path.dirname(__file__),
                              '../aux/stats.tex')
    ...
    with open(path_stats, 'w') as f:
        ...
        for var, value in stats.items():
            ...
            if type_ is int:
                f.write(f'\\def\\{var}{{{value:,}\\xspace}}\n')
            elif type_ is float:
                f.write(f'\\def\\{var}{{{value:.2f}\\xspace}}\n')
...
~\xspace~ and ~type~ detection.
    
#+begin_src python :tangle scripts/compute_stats.py
    with open("aux/stats.tex", 'a') as file_:
        file_.write(f"\\def\\{name}{{{value}\\xspace}}\n")
#+end_src

Now we define variables and write them to the file. I like to make sections.
Refresh the stats list by passing ~--refresh~ from the command line.

#+begin_src python :tangle scripts/compute_stats.py
if __name__ == "__main__":

    if "--refresh" in sys.argv:
        os.system("rm aux/stats.tex")

    sample = pd.read_csv("data/sample.csv")
    write_variable_to_latex("NSample", len(sample)

    stats['VISNIRTRANSITION'] = (SPECTRA_SETUP.VIS_NIR_TRANSITION, float)
    stats['EXTRAPOLLIMITVIS'] = (SPECTRA_SETUP.EXTRAPOL_LIMIT_VIS, float)
    stats['EXTRAPOLLIMITNIR'] = (SPECTRA_SETUP.EXTRAPOL_LIMIT_NIR, float)
    stats['Nwavesamples'] = (len(SPECTRA_SETUP.WAVE_GRID), int)
#+end_src


*** create_tables.py

This i the same prinicple as for [[compute_stats.py]].
