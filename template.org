...
#+begin_src python :tangle compute_stats.py
def write_variables_to_file(name, value):
    ...
    with open(PATH_STATS, 'w') as file_:
        ...
        for name, value in stats.items():
            ...
            if type_ is int:
                f.write(f'\\def\\{name}{{{value:,}\\xspace}}\n')
            elif type_ is float:
                f.write(f'\\def\\{name}{{{value:.2f}\\xspace}}\n')
            elif type_ is str:
                f.write(f'\\def\\{name}{{{value}\\xspace}}\n')
...
if __name__ == "__main__":
    ...
    data = pd.read_csv("data/release_dates.csv")
...
This follows the same prinicple as [[compute_stats.py]].
